...
static void cu__hash(struct cu *cu, struct tag *tag, struct conf_load *conf)
{

	// The whole alt file will be loaded at alt_cu sharing the ptr_tables
	// with the main cu, then we will hash it there, we can't hash it on
	// the main CU as the offsets are for the alt one.

	if (!(!conf->from_imported_unit_expansion_from_alt || conf->alt_dwarf))
		return;
...
static struct dwarf_off_ref attr_type(Dwarf_Die *die, uint32_t attr_name, struct conf_load *conf)
{
	Dwarf_Attribute attr;
...
static void tag__init(struct tag *tag, struct cu *cu, Dwarf_Die *die, struct conf_load *conf)
{
	struct dwarf_tag *dtag = tag->priv;
	...
	if (tag->tag == DW_TAG_imported_module ||
		...
		dtag->type = attr_type(die, DW_AT_import, conf);
	else
		dtag->type = attr_type(die, DW_AT_type, conf);

	dtag->abstract_origin = attr_type(die, DW_AT_abstract_origin, conf);
							  ...
							  struct cu *cu, struct conf_load *conf)
{
	...
	if (ptr != NULL) {
		tag__init(&ptr->tag, cu, die, conf);
		struct dwarf_tag *dtag = ptr->tag.priv;
		dtag->containing_type = attr_type(die, DW_AT_containing_type, conf);
	}
...
static struct array_type *array_type__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	struct array_type *at = tag__alloc(cu, sizeof(*at));

	if (at != NULL) {
		tag__init(&at->tag, cu, die, conf);
		at->dimensions = 0;
		at->nr_entries = NULL;
...
static struct string_type *string_type__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	struct string_type *st = tag__alloc(cu, sizeof(*st));

	if (st != NULL) {
		tag__init(&st->tag, cu, die, conf);
...
static void namespace__init(struct namespace *namespace, Dwarf_Die *die,
			    struct cu *cu, struct conf_load *conf)
{
	tag__init(&namespace->tag, cu, die, conf);
...
static void type__init(struct type *type, Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	dwarf_tag__set_spec(type->namespace.tag.priv,
			    attr_type(die, DW_AT_specification, conf));
...
static struct enumerator *enumerator__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	if (enumerator != NULL) {
		tag__init(&enumerator->tag, cu, die, conf);
...
static struct variable *variable__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	if (var != NULL) {
		tag__init(&var->ip.tag, cu, die, conf);
					      ...
					      bool in_union, struct conf_load *conf)
{
	...
	if (member != NULL) {
		tag__init(&member->tag, cu, die, conf);
					...
					struct conf_load *conf, int param_idx)
{
	...
	if (parm != NULL) {
		...
		tag__init(&parm->tag, cu, die, conf);
...
static struct label *label__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	if (label != NULL) {
		tag__init(&label->ip.tag, cu, die, conf);
...
static struct lexblock *lexblock__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	struct lexblock *block = tag__alloc(cu, sizeof(*block));

	if (block != NULL) {
		tag__init(&block->ip.tag, cu, die, conf);
...
static void ftype__init(struct ftype *ftype, Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
#ifndef NDEBUG
	const uint16_t tag = dwarf_tag(die);
	assert(tag == DW_TAG_subprogram || tag == DW_TAG_subroutine_type);
#endif
	tag__init(&ftype->tag, cu, die, conf);
...
static struct ftype *ftype__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	struct ftype *ftype = tag__alloc(cu, sizeof(*ftype));

	if (ftype != NULL)
		ftype__init(ftype, die, cu, conf);
...
static struct function *function__new(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	if (func != NULL) {
		ftype__init(&func->proto, die, cu, conf);
...
static struct tag *die__create_new_tag(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	struct tag *tag = tag__new(die, cu, conf);
...
static struct btf_type_tag_ptr_type *die__create_new_btf_type_tag_ptr_type(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
					       ...
					       struct conf_load *conf)
{
	...
	do {
		...
		if (tag == NULL) {
			...
			tag = die__create_new_btf_type_tag_ptr_type(die, cu, conf);
			if (!tag)
				return NULL;
		}
	...
	return tag ? &tag->tag : tag__new(die, cu, conf);
}

static struct tag *die__create_new_ptr_to_member_type(Dwarf_Die *die,
						      struct cu *cu, struct conf_load *conf)
{
	struct ptr_to_member_type *ptr = ptr_to_member_type__new(die, cu, conf);
...
static struct tag *die__create_new_array(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	struct array_type *array = array_type__new(die, cu, conf);
...
static struct tag *die__create_new_string_type(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	struct string_type *string = string_type__new(die, cu, conf);
...
{
	Dwarf_Die child;
	struct ftype *ftype = ftype__new(die, cu, conf);
	struct tag *tag;
...
{
	struct lexblock *lexblock = lexblock__new(die, cu, conf);
				      ...
				      int top_level, const char *fn, struct conf_load *conf)
{
	...
	case DW_TAG_string_type: // FORTRAN stuff, looks like an array
		tag = die__create_new_string_type(die, cu, conf); break;
	case DW_TAG_base_type:
	...
	case DW_TAG_ptr_to_member_type:
		tag = die__create_new_ptr_to_member_type(die, cu, conf); break;
	case DW_TAG_enumeration_type:
...
static int __die__process_unit(Dwarf_Die *die, struct cu *cu, bool from_imported_unit_expansion_from_alt, struct conf_load *conf)
{
	...
	// file and thus we need to mark their offsets as such so that lookups
	// will use the alternative DWARF offset keyed hashtables (in struct dwarf_cu->alt),
	// otherwise their offsets will clash with the offsets in the main CU.

	// And since we add subtags (DW_TAG_member, DW_TAG_formal_parameter,
	// DW_TAG_variable, etc we can't just set its dtag->type.from_alt to
	// true here, we need to set conf->from_imported_unit_expansion_from_alt
	// to signal that and then have it set in attr_type().
...
static int __cus__finalize(struct cus *cus, struct cu *cu, struct cu *alt_cu, struct conf_load *conf, void *thr_data)
{
	if (alt_cu)
		cu__for_all_tags(alt_cu, class_member__cache_byte_size, conf);
...
{
	return __cus__finalize(cus, cu, NULL, conf, thr_data);
}
...
{
	// FIXME we either move this direction or load CU by CU and have a first level lookup tree to 
	// do inter-CU lookups as needed with LTO and in some cases whe .dwz is used.
	//
	// For now lets leave it this way but then this precludes -jN parallel loading of vmlinux for
	// BTF encoding, a major use case :-(
...
static struct cu *cu__new_alt(struct conf_load *conf, Dwarf *alt_dw, Dwfl_Module *mod, Elf *elf, struct tag_tables *tables)
{
	...
	while (dwarf_nextcu(alt_dw, off, &noff, &cuhl, NULL, &pointer_size, &offset_size) == 0) {
		...
		if (cu == NULL) {
			...
			if (cu == NULL || cu__set_common(cu, &alt_conf, mod, elf) != 0)
				goto out_abort;
				     ...
				     struct dwarf_cu *type_dcu)
{
	...
	// See if we have an alternate DWARF associated, i.e. if we have
	// DW_TAG_imported_unit and DW_FORM_GNU_ref_alt references to a .dwz file
	Dwarf *alt_dw = dwarf_getalt(dw);

	if (alt_dw != NULL) {
		alt_cu = cu__new_alt(conf, alt_dw, mod, elf, cu->tables);
		if (alt_cu == NULL)
			goto out_abort;

		// Remember the alt_cu dwarf_cu instance so that at dwarf_cu__find_{tag,type}_by_ref()
		// We can look at dwarf_tag->type.from_alt and if it is true, use dcu->alt instead
		// of dcu to look at dwarf_cu's hash_{tags,types}
		if (dcu->alt == NULL)
			dcu->alt = alt_cu->priv;
	...
	if (__cus__finalize(cus, cu, alt_cu, conf, NULL) == LSK__STOP_LOADING)
		goto out_abort;
